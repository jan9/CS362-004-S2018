...
* Author: Ju An
* Date: April 29, 2018
* OSU CS 362 - Assignment 3: Card Test 3
* This is a unit test for Council Room card in dominion.c
*/
...
int main()
{
	...
	failure  += asserttrue (savedData.handCount[player1]+3, G.handCount[player1]);
	printf("\n   Checking player 1's deckCount\n");
	failure  += asserttrue (savedData.deckCount[player1]-4,G.deckCount[player1]);
	...
	printf("\n   Prev # of discard count = %d\n", savedData.numBuys);
	printf("\n   Current # of discard count = %d\n", G.numBuys);
	failure  += asserttrue (savedData.numBuys+1,G.numBuys);
	
	printf("\nTEST 4. Check if player 2's gets to draw one card for free\n");
	printf("\n   Checking player 2's handCount\n");
	failure  += asserttrue (savedData.handCount[player2]+1,G.handCount[player2]);
	printf("\n   Checking player 2's deckCount\n");
	failure  += asserttrue (savedData.deckCount[player2]-1,G.deckCount[player2]);
